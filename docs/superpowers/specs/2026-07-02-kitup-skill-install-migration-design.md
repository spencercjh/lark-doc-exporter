...
Installed targets are now owned through `kitup`'s `.kitup.json` metadata
...
For upgrade compatibility, the adapter should still recognize legacy managed
installs written by the previous `lark-doc-exporter` installer. The migration
bridge should:

- read `.lark-doc-exporter-install.json`
- verify `tool == "lark-doc-exporter"`
- seed compatible `.kitup.json` ownership metadata for planning/install
- preserve `dry-run` cleanliness by removing any temporary seeded metadata when
  no real install happens

That keeps the first `kitup`-backed upgrade on the managed path instead of
dropping old installs into an unnecessary `--force` conflict.
...
Current validated state on the branch:

- targeted installer/CLI tests pass
- full repo `pytest` passes
- GitHub Actions checks for PR #14 pass after pinning `uv` in workflow setup
